param(
    [string]$TargetDir = "src\assets\images\cards_ssr",
    [int]$TargetSize = 512,
    [int]$Quality = 85
)
...
function Optimize-SingleCard {
    ...
    Write-Host "Optimized: $([System.IO.Path]::GetFileName($fullPath))"
}

$fullTarget = [System.IO.Path]::GetFullPath($TargetDir)
if (Test-Path $fullTarget) {
    $files = Get-ChildItem -Path $fullTarget -Filter "*.jpg"
    foreach ($f in $files) {
        Optimize-SingleCard -Path $f.FullName -TargetSize $TargetSize -Quality $Quality
    }
    Write-Host "Done optimizing $($files.Count) files in $TargetDir"
}
