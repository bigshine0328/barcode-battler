...
2. **ルート2: 明確なコードバグ・障害対応 (Hotfix / Bug Fix)**
   - 担当の流れ: `software-engineer` (原因分析・コード修正 & **`docs/Basic_Design.md` の同期更新**) ➡️ `code-reviewer` (コード & 設計同期レビュー) ➡️ `qa-engineer` (修正検証) ➡️ **ユーザー承認** ➡️ `release-engineer` (パッチリリース)
...
- **ドキュメント非破壊的改定 & 差分監査の義務化**:
  ...
  - 機能削除を伴う改定の場合、「①ユーザー指示との合致」「②改定履歴（Changelog）への理由明記」「③無関係な仕様の巻き添え削除がないこと」を `requirements-reviewer` と連携して監査し、無断欠落・デグレがないことを確認してからユーザー承認ゲートへ提出すること。
