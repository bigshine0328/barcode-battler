...
4. **UI/UX仕様書の改定・機能削除ルール (Change Requests / Maintenance)**:
   ...
   - **【安全な更新手順 (Safe Update Procedure)】**:
     1. **全文書き直し禁止**: UI仕様書改定時、`docs/UI_Design.md` の全文を一度に書き直すことを禁止する。変更対象の画面・セクションのみを外科的に編集すること。
     2. **セクション棚卸し**: 改定前に全セクション見出し（`## ` 以上）と全画面ID（SCR-XX）をリスト化し、改定後に照合する。1つでも見出しまたは画面IDが消えていれば即座に復元すること。
     3. **定量データ保全検証**: 改定後、変更対象外のワイヤーフレーム・カラーコード・モーション秒数・コンポーネント仕様が改定前と完全一致していることを確認すること。
