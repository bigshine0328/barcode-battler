...
## 改定履歴 (Changelog)
- **v4.4.0 (2026-08-28)**:
  - 【新機能】ホーム画面（`SCR-01`）ヘッダーに「⚙️ 設定」ボタンを追加。
  - 【新機能】「⚙️ データ管理・バックアップモーダル（`#settings-data-modal`）」のUI/UXワイヤーフレームを設計（JSONダウンロード、テキストコードコピー、ファイルアップロード選択、コード貼付復元、確認ダイアログ）。
- **v4.3.0 (2026-08-24)**:
  - 【UI改善】全128枚世界観背景付きTCGアート配備完了に伴う属性オーラ削除およびレアリティ枠演出一本化。メインキャラ展示エリアの縦幅を拡張し文字被りを解消。
- **v3.8.0 (2026-08-16)**:
  - 【新機能・互換性拡張】SCR-02 スキャン画面において、iOS Safari（iPhone）実機でのカメラ直接スキャンを完全サポート。
  - 【改善】Android Chrome等では既存のネイティブスキャンを完全維持し、iOS環境でのみキャンバス解析（ZXing）へ透過的にフォールバックするゼロインパクトUI/UXを設計。
- **v3.7.0 (2026-08-15)**:
  - 【新機能・改善】SCR-06 バトル画面において、デッキでメイン・サブが未セットの場合でも所持図鑑からランダム選抜されたキャラがスムーズに出撃・描画されるUXフローを定義。
  - 【改善】SCR-05 対戦ロビーにおいて、ゲスト参加者はホストが決定した対戦ルール（1P/3P）に自動追従して接続・画面遷移するシームレスUXを確立。
- **v3.6.0 (2026-08-15)**:
  - 【不具合修正】SCR-06 3P対戦において、ゲスト側のサブスロット（次鋒・大将）に意図しないアイテム（いあつのたて）が表示される不具合を修正し、正規のキャラクターカードが常に表示されるよう修正。
  - 【不具合修正】SCR-06 バトル画面において、「にげる」実行時および通信切断時に自分・相手双方の端末がダイアログ表示と共に対戦ロビー（`SCR-05`）へ自動復帰するUX遷移を保証。
- **v3.5.0 (2026-08-15)**:
...
### 5.5 データ管理・バックアップモーダル (`#settings-data-modal`)
ホーム画面（`SCR-01`）右上の「⚙️」タップ時に表示：
- **ヘッダー**: `⚙️ データ管理 & 引き継ぎ (v4.4.0)` ＋ `[× 閉じる]`
- **📥 バックアップ（出力）セクション**:
  - `[📥 バックアップファイルを保存 (JSON)]` : ワンタップでファイルダウンロード。
  - `[📋 バックアップコードをコピー]` : クリップボードへコピーし「コピーしました！」トースト表示。
- **📤 復元・引き継ぎ（入力）セクション**:
  - `[📂 バックアップファイルを選択して復元]` : ファイルピッカー起動。
  - バックアップコード貼り付け用テキストエリア ＋ `[🔄 コードから復元]`。
- **安全確認ダイアログ**:
  - リストア実行時に `⚠️ 現在の所持データが上書きされます。よろしいですか？` を表示し、同意後に即時再描画。
